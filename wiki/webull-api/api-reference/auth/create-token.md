...
title: Create Token
method: POST
endpoint: /openapi/auth/token/create
rate_limit: 10 requests every 30 seconds
source: https://developer.webull.hk/apis/docs/reference/create-token
updated: 2026-05-17
...
```json
{
  ...
  "description": "• Function description: Create an access token.This interface is used to generate a new Token, which is the credential for accessing other API interfaces. Upon successful creation, it returns a response containing Token information, expiration time, and status. The Token status defaults to 'Pending Verification' and requires verification via Webull App SMS code. Tokens are time-sensitive (default 15 days) and need to be refreshed before expiration.  \n• Frequency limit: Rate limit 10 requests every 30 seconds",
  ...
  "postman": {
    "name": "Create Token",
    "description": {
      "content": "• Function description: Create an access token.This interface is used to generate a new Token, which is the credential for accessing other API interfaces. Upon successful creation, it returns a response containing Token information, expiration time, and status. The Token status defaults to 'Pending Verification' and requires verification via Webull App SMS code. Tokens are time-sensitive (default 15 days) and need to be refreshed before expiration.  \n• Frequency limit: Rate limit 10 requests every 30 seconds",
      "type": "text/plain"
    },
    "url": {
      "path": [
